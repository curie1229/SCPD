...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  ### START CODE HERE
  y_hat = softmax(np.dot(center_word_vec.reshape(1, -1), np.transpose(outside_vectors)))
  loss = -np.log(y_hat[0][outside_word_idx])
  delta = y_hat.copy() # y_hat - y [1, n]
  delta[0][outside_word_idx] = delta[0][outside_word_idx] - 1
  grad_center_vec = np.dot(delta, outside_vectors)
  grad_outside_vecs = np.dot(np.transpose(delta), center_word_vec.reshape(1, -1))
  ### END CODE HERE
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  for word in outside_words:
        ...
        current_loss, current_grad_center_vecs, current_grad_outside_vectors = word2vec_loss_and_gradient(
                                center_word_vec=center_word_vectors[word2ind[current_center_word]],
                                outside_word_idx=outside_word_idx,
                                outside_vectors=outside_vectors,
                                dataset=dataset)
        loss += current_loss
        grad_center_vecs[word2ind[current_center_word]] += current_grad_center_vecs.reshape(-1)
        grad_outside_vectors += current_grad_outside_vectors
  ### END CODE HERE
